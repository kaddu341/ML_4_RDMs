...
# first, we need to get the normalization constants
def get_normalization_constants(filenames_dict):    # filenames_dict should be of form "filepath" : # of training samples
    stats = dict()
    ...
    for filename in filenames_dict.keys():
        with h5py.File(filename, 'r') as f:
            ...
            for i in range(num_samples):
                sample = f['dataset'][i]
                initial_arr = sample[0]
                results_arr = sample[1]
                # get sum of elements
                x_sum += np.sum(initial_arr)
                y_sum += np.sum(results_arr)
                # get num of elements
                assert initial_arr.size == results_arr.size
                num_elements += initial_arr.size
    
    stats["x_mean"] = (x_sum / num_elements).item()
    stats["y_mean"] = (y_sum / num_elements).item()
    ...
    for filename in filenames_dict.keys():
        with h5py.File(filename, 'r') as f:
            ...
            for i in range(num_samples):
                ...
                initial_arr = sample[0]
                results_arr = sample[1]
                # get difference w.r.t. mean
                initial_arr = initial_arr - stats["x_mean"]
                results_arr = results_arr - stats["y_mean"]
                # square the result
                initial_arr = initial_arr ** 2
                results_arr = results_arr ** 2
                # take the sum
                x_variance += np.sum(initial_arr)
                y_variance += np.sum(results_arr)
    # divide by num_elements - 1 to get the true variance
    x_variance = x_variance / (num_elements - 1)
    y_variance = y_variance / (num_elements - 1)
    # take the square root to get the standard deviation
    stats["x_std"] = np.sqrt(x_variance).item()
    stats["y_std"] = np.sqrt(y_variance).item()

    # write normalization constants to a JSON file so that we can load them later
    with open("normalization_constants.json", "w") as f:
        json.dump(stats, f)

    return stats["x_mean"], stats["x_std"], stats["y_mean"], stats["y_std"]

def get_normalization_constants_3x3(filenames_dict, write_to="normalization_constants.json"):    # filenames_dict should be of the same form as above
    stats = dict()
    # first get the averages
    x_sum = 0.0
    y_sum = 0.0
    ...
    for filename in filenames_dict.keys():
        with h5py.File(filename, 'r') as f:
            num_samples = filenames_dict[filename]
            initial_data = f['dataset'][:num_samples, 0, :, :]
            results_data = f['dataset'][:num_samples, 1, :, :]
    ...
    stats["x_mean"] = (x_sum / x_num_elements).item()
    stats["y_mean"] = (y_sum / y_num_elements).item()
    ...
    for filename in filenames_dict.keys():
        with h5py.File(filename, 'r') as f:
            ...
            initial_data = initial_data - stats["x_mean"]
            results_data = results_data - stats["y_mean"]
...
# dataset class for when you need a very large .h5 file
class HartreeFockDatasetHDF5(Dataset):
    def __init__(
        self,
        filename,
        normalization_constants: dict = None,
        indices: Tuple[int, int] = None,
    ):
        self.filename = filename
        self.file = None  # <- Not opened yet

        # get indices to partition the dataset
        if indices is None:
            with h5py.File(filename, 'r') as f:
                full_length = len(f['dataset'])
            self.start_index = 0
            self.end_index = full_length
        else:
        ...
        # other attributes
        self.transform = lambda x: normalize(x, x_mean, x_std)
        self.target_transform = lambda y: normalize(y, y_mean, y_std)
    ...
    def __getitem__(self, index):
        # Lazily open file the first time __getitem__ is called
        if self.file is None:
            self.file = h5py.File(self.filename, 'r')
        
        # first get the actual index relative to the dataset
        index = self.start_index + index
        
        # now read from the H5 file:
        sample = torch.from_numpy(self.file['dataset'][index])
        initial_arr = sample[0]
        results_arr = sample[1]
...
class HF_3x3_Dataset(Dataset):
    def __init__(self, filename, normalization_constants: dict = None, indices: Tuple[int, int] = None, transform = None, target_transform = None):
        ...
        self.initial_data = normalize(self.initial_data, x_mean, x_std)
        self.results_data = normalize(self.results_data, y_mean, y_std)
