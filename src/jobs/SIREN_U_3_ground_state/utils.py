...
def update_best_model_guarded(foldername, model, max_diff):
    """
    Updates the best model if max_diff is lower than the recorded best.
    ...
    with open(json_path, "a+") as f:
        ...
        try:
            current = json.load(f)
            current_best = current.get("best_diff", float("inf"))    # inf is just the default value
        except (json.JSONDecodeError, FileNotFoundError):
            current_best = float("inf")
        
        # Update only if we get a better test loss
        if max_diff < current_best:
            try:
                ...
                with open(tmp_meta, "w") as tmpf:
                    json.dump({"best_diff": max_diff}, tmpf)
                os.replace(tmp_meta, json_path)

                print(f"[INFO] Updated best model: Max Diff={max_diff:.4e}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (epoch + 1) % 10 == 0:  # checkpoint every 10 epochs
            ...
            if max_diff < MIN_DIFF:
                MIN_DIFF = max_diff
                update_best_model_guarded(foldername, model, max_diff)
            # ==========================
        else:
...
def matrix_processing(
    ...
    checkpoint_config = tune.CheckpointConfig(
        ...
        checkpoint_score_attribute="max_diff", # <-- The metric to judge "best"
        checkpoint_score_order="min" # <-- "min" because lower max_diff is better
    )
